...
export default function Login() {
  ...
  const { loading, signIn, setUser, darkMode } = useAuth()
      ...
      <div className={`inline-flex justify-center items-center w-screen dark:bg-dark-bg  h-screen font-montserrat`}>
        ...
        <Formik initialValues={{ phoneNo: '', password: ''}} validationSchema={validationSchema}>
          {({values, errors, touched, handleChange, handleBlur}) => {
            return (
              <form onSubmit={(event) => handleSubmit(event, values)} className='w-11/12 p-10 sm:w-8/12 md:w-6/12 lg:w-4/12 bg-white dark:bg-dark-bg-700 shadow-myShadow flex justify-center items-center flex-col rounded-lg'>
